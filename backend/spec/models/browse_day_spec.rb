...
RSpec.describe BrowseDay, type: :model do
  ...
  it 'is valid with user_id, page_id and day' do
    browse_day = BrowseDay.new(user_id: @user.id, page_id: @page.id, day: 1)
  ...
  it 'dose not allow same [user_id, page_id, day]' do
    BrowseDay.create(user_id: @user.id, page_id: @page.id, day: 1)
    browse_day = BrowseDay.new(user_id: @user.id, page_id: @page.id, day: 1)
  ...
  it 'is valid different day' do
    BrowseDay.create(user_id: @user.id, page_id: @page.id, day: 1)
    browse_day = BrowseDay.new(user_id: @user.id, page_id: @page.id, day: 2)
    expect(browse_day).to be_valid
  end
end
